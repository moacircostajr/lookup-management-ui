...
export default function TabelaEmpresas() {
  ...
  useEffect(() => {
      api.get('companies', {headers: {Authorization: token}}).then(response => {
  	...
  	<div>
	    <TableContainer component={Paper} className={classes.container}>
	      <Table className={classes.table}>
	        <TableHead>
	          <TableRow>
              <StyledTableCell align="center">Nome de Fantasia</StyledTableCell>
              ...
              <StyledTableCell align="center"><Button variant="contained" color="inherit" onClick={() => { history.push('/empresa') }}><AddIcon color="primary"/></Button></StyledTableCell>
	          </TableRow>
	        </TableHead>
	        <TableBody>
	          {
              dados.map((valor, chave) => (
              <StyledTableRow hover key={chave} onClick={ () => history.push('/empresa/' + valor['id']) }> 
                <StyledTableCell align="center">{valor['business_name']}</StyledTableCell>
                <StyledTableCell align="center">{valor['company_name']}</StyledTableCell>
